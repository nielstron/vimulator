/*
 * VimulatorPlugin.java - Vimulator plugin
 * Copyright (C) 2000, 2001, 2002 mike dillon
 *
...
public class VimulatorPlugin extends EBPlugin
{
	...
	public void handleMessage(EBMessage msg)
	{
		...
		else if (msg instanceof BufferUpdate)
		{
			...
			if (bmsg.getWhat() == BufferUpdate.LOADED)
			{
				...
				updateBufferStatus(buffer, jEdit.getProperty(
					"vimulator.msg.simple-status", args), null);
			}
		}
		else if (msg instanceof EditPaneUpdate)
		{
			...
			if (emsg.getWhat() == EditPaneUpdate.BUFFER_CHANGED)
			{
				...
				if (buffer.isNewFile())
				{
					updateBufferStatus(buffer, null,
						forViews);
					return;
				}
				...
				updateBufferStatus(buffer, jEdit.getProperty(
					"vimulator.msg.simple-status", args),
					forViews);
			}
	...
	{
		return "Vimulator " + jEdit.getProperty(
			"plugin.vimulator.VimulatorPlugin.version");
	}
	...
	public static void setLastFindChar(char ch, boolean reverse,
		boolean until)
	{
	...
	// private members
	private static VimulatorInputHandler rootHandler
		= new VimulatorInputHandler(null);
	...
	class VimulatorCaretListener implements CaretListener
	{
		...
		{
			if (!VimulatorUtilities.checkEmulation(view)) return;

			int lastAllowed = VimulatorUtilities
				.getLastAllowedOffset(view, textArea);
			if (textArea.getCaretPosition() <= lastAllowed) return;
